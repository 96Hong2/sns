...
public class PostControllerTest {
    ...
    void 댓글기능() throws Exception {
        mockMvc.perform(post("/api/v1/posts/1/comments")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsBytes(new PostCommentRequest("comment"))) // 댓글 요청에 대한 content 받음
                ).andDo(print())
                .andExpect(status().isOk());
    }
    ...
    void 댓글작성시_게시물이_없는경우() throws Exception {
        // mocking : postService의 comment를 실행했을 때 POST_NOT_FOUND 에러를 던지도록 모킹
